...
private final String vertexShaderCode =
    ...
    private final String fragmentShaderCode =
            "precision mediump float;        \n" +
                    ...
                    "void main()\n" +
                    "{\n" +
                    "    gl_FragColor = texture2D(u_Texture, gl_PointCoord) + vec4(v_Color,0.0);\n" +
                    "  }";
    ...
    public Vector() {
        ...
        for(int i=0;i<100;i++) {
            p[i] = new Particle();
            p[i].nextX=rand.nextInt(10);
            p[i].nextY=rand.nextInt(10);
            p[i].z=i;
            for(int j=0;j<3;j++) {
        ...
        GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, vertexBuffer[0].capacity() * 4,
                vertexBuffer[0], GLES20.GL_DYNAMIC_DRAW);
    ...
    public void draw(float[] mvpMatrix) {
        ...
        for(int i=0;i<100;i++) {
            p[i].time+=(float)rand.nextInt(10)/1000;
            if(Math.round(p[i].x) == p[i].nextX) {
                p[i].time=0;
                p[i].nextX=rand.nextInt(10);
                p[i].nextY=rand.nextInt(10);
                //p[i].nextZ=rand.nextInt(10);
                for(int j=0;j<3;j++) {
        ...
        GLES20.glBufferSubData(GLES20.GL_ARRAY_BUFFER,0, vertexBuffer[0].capacity() * 4,
                vertexBuffer[0]);
        ...
        GLES20.glBufferSubData(GLES20.GL_ARRAY_BUFFER,0, colorBuffer[0].capacity() * 4,
                colorBuffer[0]);
