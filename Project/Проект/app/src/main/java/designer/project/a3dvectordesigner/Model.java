...
private final String vertexShaderCode =
    ...
    private static FloatBuffer[] normalBuffer = new FloatBuffer[100];
    ...
    public static Bitmap[] textures = new Bitmap[100];
    int temp=0;
    ...
    public Model() {
        ...
        GLES20.glGenBuffers(100, nbos, 0);
        ...
        for(int i=0;i<br;i++){
            ...
            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, nbos[i]);
            //TEXTURE
            GLES20.glBufferData(GLES20.GL_ARRAY_BUFFER, colorBuffer[i].capacity() * 4,
                    colorBuffer[i], GLES20.GL_STATIC_DRAW);
            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0);
    ...
    public static void loadNormBuff(float[] modelNormals){

        ByteBuffer bb = ByteBuffer.allocateDirect(
                // (# of coordinate values * 4 bytes per float)
                (modelNormals.length * 4));
        bb.order(ByteOrder.nativeOrder());
        normalBuffer[br4] = bb.asFloatBuffer();
        normalBuffer[br4].put(modelNormals);
        normalBuffer[br4].position(0);

        br4++;
    }

    public void draw(float[] mvpMatrix, int modelNum, int textureNum) {
        ...
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, texHandle[textureNum]);
        ...
        GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, nbos[modelNum]);
        GLES20.glVertexAttribPointer(
